namespace Test
{
    ...
    public class Benchmark
    {
        ...
        {
            return _keyValuePairs.Select(item => item.Key).ToList().GetRange(0, Math.Min(_keyValuePairs.Count, 5));
        }

        [Benchmark]
        public List<string> GetRangeDotSelectDotToList()
        {
            return _keyValuePairs.GetRange(0, Math.Min(_keyValuePairs.Count, 5)).Select(item => item.Key).ToList();
        }
